...
-- staff_id 1번인 사람은 salary = 5000을 넣어주시면 됩니다.
-- 2 4500
-- 3 5500
-- 4 4800
-- 5 4700
...
update staff set salary = 4500 where staff_id = 2;
update staff set salary = 5500 where staff_id = 3;
update staff set salary = 4800 where staff_id = 4;
update staff set salary = 4700 where staff_id = 5;

-- sum()
-- 전체 직원의 급여 합산 계산
select sum(salary) as total_salary from staff;
-- select sum(칼럼명) as 별칭 from 테이블이름;

-- store_id별로 급여 합계 계산
select store_id, sum(salary) as total_salary
from staff
group by store_id;

-- 특정 성씨를 가진 직원들의 급여 합계 계산
-- last_name 'johnson'
...
-- 소수 2째 자리 수까지만 표현하고 싶을 때 / 반올림 해서
select round(avg(salary), 2) as average_salary from staff;

-- store_id별로 평균 급여 계산
select store_id, avg(salary) as average_salary
from staff
group by store_id;

-- 소수 2째 자리까지만 남기고 절삭하고 싶어요 / 반올림 x
select truncate(avg(salary), 2) as average_salary
from staff
group by store_id;
...
-- truncate() : 소수점 이하 특정 자리까지만 남기고 나머지를 버리고 싶을 때 사용
-- floor() : 소수점 이하는 모두 버리고 정수만 반환

-- store_id가 1인 직원들의 평균 급여를 소수점 이하 3째 자리에서 반올림한 값을 계산하여 반환하시오.
select store_id, round(avg(salary), 3) as average_salary
from staff
where store_id = 1   -- 조건
group by store_id;

-- 특정 조건을 만족하는 그룹만 선택
select store_id, round(avg(salary), 1) as avg_sa
from staff
group by store_id
having avg(salary) > 5000;   -- having절도 조건절이다.

-- 동일한 성씨를 가진 직원이 2명 이상인 성씨 그룹을 찾고, 각 그룹의 직원 수를 반환한다.
select last_name, count(*) as employee_count
from staff
group by last_name
having count(*) > 1;

-- 각 store_id 별로 직원들의 급여 총합을 계산한 후, 그 총합이 10000 이상인 상점만을 반환한다.
select store_id, sum(salary) as total_s
from staff
group by store_id
having sum(salary) >= 10000;

-- 최소 급여가 4700 이상인 성씨 그룹 찾기
select last_name, min(salary) as min_l
from staff
group by last_name
having min(salary) >= 4700;

-- 종합 예제
-- 1. 직원들의 성씨를 모두 대문자로 변경하고, 그들의 성씨를 기준으로 그룹화하여 성씨별 직원 수를 구하시오.
select upper(last_name) as up_name, count(*) as employee_count
from staff
group by last_name;

-- 2. 전체 직원의 급여 총합을 구한 후, 그 값을 1000 단위로 나눈 결과를 계산하시오.
-- 단, 소수 2째자리 수까지만 표현하기
select round(sum(salary) / 1000, 2) as salary_in
from staff;


-- 3. last_name이 "johnson"인 직원들의 급여를 합산한 후, 합산된 급여가 10000보다 큰 결과 값만 반환하기
select last_name, sum(salary) as total_salary
from staff
where last_name = 'johnson'
group by last_name
